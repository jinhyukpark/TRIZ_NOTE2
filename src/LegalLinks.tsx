...
export function ManageSubscription(){const{locale}=useLanguage();const open=useLegalLink();return <View style={{gap:8}}><Button secondary title={copy.manage[locale]} onPress={()=>void open(Platform.OS==='ios'?'https://apps.apple.com/account/subscriptions':'https://play.google.com/store/account/subscriptions')}/><Text style={s.muted}>{copy.manageInfo[locale]}</Text></View>;}
