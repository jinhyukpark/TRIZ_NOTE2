...
import {checkDetailAccess,detailAccessCopy} from './src/lib/detailAccess';
...
function Main(){
 ...
 const accessRequest=useRef(0),accessBusy=useRef(false);
 useEffect(()=>()=>{accessRequest.current++;accessBusy.current=false;},[session?.user.id]);
 const subscription=useSubscription(session?.user.id);
 // Menus remain browsable; every content detail requires a paid entitlement.
 const gated=detail!==null||collection!==null||effect!==null;
 ...
 function navigate(next:string){accessRequest.current++;accessBusy.current=false;setEffect(null);setDetail(null);setCollection(null);setTab(next);}
 function openContent(open:()=>void){
  if(accessBusy.current)return;
  accessBusy.current=true;const request=++accessRequest.current;const copy=detailAccessCopy[locale];
  void checkDetailAccess(subscription.refresh,open,error=>Alert.alert(copy.title,error?copy.error:copy.message,[
   {text:copy.cancel,style:'cancel'},
   {text:copy.subscribe,onPress:()=>navigate('store')},
  ]),()=>request===accessRequest.current).finally(()=>{if(request===accessRequest.current)accessBusy.current=false;});
 }
 const openDetail=(id:number)=>openContent(()=>setDetail(id));
 const openCollection=(value:Collection)=>openContent(()=>setCollection(value));
 const openEffect=(id:string|null)=>{if(id===null)setEffect(null);else openContent(()=>setEffect(id));};
 ...
 return <SafeAreaView style={s.root}><StatusBar style="light"/>{!detail&&!(tab==='effects'&&effect)&&<View style={[s.between,{flexWrap:'wrap',paddingHorizontal:20,paddingVertical:10,borderBottomWidth:1,borderColor:colors.line}]}><BrandLogo/><Pressable accessibilityRole="button" onPress={()=>navigate('account')} style={{padding:12}}><Text style={{color:colors.ink}}>{t('설정')} ⚙</Text></Pressable></View>}
