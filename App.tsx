...
import LegalLinks,{ManageSubscription} from './src/LegalLinks';
...
function Main(){
 ...
 const subscription=useSubscription(billingEnabled?session?.user.id:undefined);
 ...
 if(recovery||(!session&&!guest))return <SafeAreaView style={s.root}><StatusBar style="light"/><Auth recovery={recovery} onRecovered={()=>setRecovery(false)}/>{!recovery&&<View style={{padding:16}}><Button secondary title={t('학습 자료 둘러보기')} onPress={()=>setGuest(true)}/></View>}</SafeAreaView>;
