...
const {effects}=await load(compile('src/data/generatedEffects.ts')+'\n'+compile('src/data/effects.ts').replace(/import \{ generatedEffects \} from '.\/generatedEffects';\n/,''));
...
test('every existing Effect has valid independent kind, fields and function metadata',()=>{
 ...
 assert.equal(filterEffects(effects,'','all','basic').length,7);
 assert.equal(filterEffects(effects,'','all','application').length,5);
});
test('filters combine with AND, include secondary fields, and support multilingual functional search',()=>{
 ...
 assert.deepEqual(ids(filterEffects(effects,'','acoustic','application')),['ultrasonic-soldering','ultrasonic']);
 assert.ok(ids(filterEffects(effects,'','material','application')).includes('ultrasonic-soldering'));
 assert.deepEqual(ids(filterEffects(effects,'냉각','all','all')),['heat-pipe']);
