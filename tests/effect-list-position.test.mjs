...
test('Effects restores the previous list offset after detail/back without resetting search or filters',()=>{
 ...
 const dependencies={
  react:hooks,'react-native':{View:'View',ScrollView:'ScrollView',Pressable:'Pressable',TextInput:'TextInput',Modal:'Modal',Keyboard:{dismiss(){}},StyleSheet:{create:s=>s},useWindowDimensions:()=>({width:390})},
  ...
  '../lib/effectContent':{refreshEffectContent:async()=>0},
  '../i18n':{useLanguage:()=>({locale:'ko'})},'../theme':{s:{},colors:{}},
 };
 ...
 nodes(tree).find(n=>n.type==='TextInput').props.onChangeText('예시');
 ...
 nodes(tree).find(n=>n.type==='Pressable'&&n.props.accessibilityLabel==='예시').props.onPress();
 ...
 const restored=[];view.props.ref.current={scrollTo:position=>restored.push(position)};
 scroll(view,0);view.props.onLayout();view.props.onContentSizeChange();
 assert.deepEqual(restored,[{y:1824,animated:false},{y:1824,animated:false}]);
